	...
	<br><hr>
		...
		<?php
						...
						foreach ($blogusers as $user) {
								...
								echo "<tr><td>" . $user->user_login . "</td><td>" . $user->user_email . "</td><td>" . $user->user_registered . "</td>";
		...
		foreach( $social as $site ){ // print_r($site->post_name);				
			...
			?>
				<td class='hidden'><a target='_blank' href='<?php echo get_field( 'website_link' , $site->ID ); ?><?php echo get_user_meta($user->ID, "MX_user_" . $site->post_name  , 1); ?>'>
					<img width='50' src='
<?php echo get_stylesheet_directory_uri(); ?>/images/icons/icon-<?php echo $site->post_name; ?>.png'  class=''> - <?php echo get_user_meta($user->ID, "MX_user_" . $site->post_name  , 1); ?>
	...
	?></td>
								...
								<td><?php 
									...
									if( substr_count($email, '.') > 3 ){
										$delete_user = 1;
										echo "<td>too many periods!</td>";
										//echo "<td>Not found!</td>";
									}
						
							$fakes = [ 'other.estabbi.com' , 'year.pa' , 'snet.net' , 'and.ac' , 'most.rag',  'i.relu' , 'hundt.' , '.ru' , '.eu' , 'farthy.com' , 'pairst.com', 'boringverse.com', '.pl', 'gotowka.com', 'intained.com', 'discopied.com', '.ml', 'traveller.com', '.club', '.website', '.network', '.gq', '.site', '.icu', '.pw', 'spambog.com', '.online', '.xyz', '.pro', 'shoars.com', '.universallightkeys.com', 'codyting.com', 'crowd.info', '.fun', 'mega.com', 'discardmail.com', 'mefound.com', 'citetick.com', 'mailid.info', 'frequiry.com', 'usamami.com', 'egogap.com', '.store', 'irant.info', 'paper.com', '.ua', 'cherry.com', 'mouses.com', 'sfxmailbox.com', '.info', '.tk', 'osting.live', 'lapex.com', 'usgeek.org', 'mondard.com', 'ringomail.info', 'root.mill', 'ukr.net', '.co.uk', '.cf', 'ranker.com', 'combb.ne', 'psis.nett', 'allads.com', 'ariots.com', 'yandex.com', '.world', 'verona.com', 'lez.org', 'bengira.com', 'in-asia.com', 'mailcatch', 'thefmail.com','.de' , 'avas.in' ];
							
							
							foreach ($fakes as $fake) {
								
								if( strpos($email, 'russ') !== false ) continue;
								//if (strstr($string, $url)) { // mine version
								if (strpos($email, $fake) !== FALSE) { // Yoshi version
									echo "<td>Match found</td>"; 
									$delete_user = 1;
									//return true;
								}else{
									//echo "<td>Not found!</td>";
						...
						if( ($age == "" || $age == "-" || $age == "N/A" || $age == "60+") ) {  }else{ 
							if( is_numeric($age) ){}else{ echo "DELETE!";
								$delete_user = 1;
								//wp_delete_user( $user->ID );
							...
							$city = get_user_meta(   $user->ID, 'MX_user_city', 1);
							if( $city == "Minsk" ){ echo "DELETE!";
								$delete_user = 1;
								//wp_delete_user( $user->ID ); 
								}
							
						} 
								echo get_user_meta(   $user->ID, 'MX_user_age', 1); ?></td>
		...
		echo "TOTAL DELETED: " . $total_delete;
					?>
	...
	// The Query
				...
				//$args4 = array( 'post_type' => 'ssi_models'  , 'posts_per_page' => $num_posts, 'orderby'=> 'modified', 'order' => $order  );
				
				//$args3 = array( 'post_type' => 'ssi_family'  , 'posts_per_page' => $num_posts, 'orderby'=> 'modified', 'order' => $order  );
				
				//$args2 = array( 'post_type' => 'ssi_clients'  , 'posts_per_page' => $num_posts, 'orderby'=> 'meta_value_num', 'meta_key' => 'client_profit' ,'order' => $order  );
				
				//$contacts = get_posts( $args );
				//$clients = get_posts( $args2 );
				//$family = get_posts( $args3 );
				//$models = get_posts( $args4 );
				//$leads = array_merge( $contacts, $models )
...
<!--  #####################   END Filter  ##############-->
	...
	<div class='col-sm-12 hidden-xs text-center'>
		...
		<div class='col-xs-3  hidden1'> Email </div>
...
<div class='contact-print container well'>
