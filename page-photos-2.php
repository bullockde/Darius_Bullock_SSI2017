...
<br>
	...
	<div class='container-fluid1'><div class='col-md-10 well1 green1'>
...
<?php
		...
		foreach( $leads as $lead ){ 
			...
			if( 0 /*check_rated( $lead->ID )*/ ){  echo "<br>" . ++$count; ?>. <a href='/?p=<?php echo $lead->ID; ?>'> <?php echo $lead->post_title; ?> </a> 
				| <small>  <?php echo get_post_meta($lead->ID, 'MX_user_city', true); ?>, <?php echo get_post_meta($lead->ID, 'MX_user_state', true); ?></small><?php echo "Already Rated!"; continue; } 
		...
		<div class='video-set col-md-12 well'>
			...
			//	echo "<center>" . get_field( 'member_level', $lead->ID ) . "</center>";
    			...
    			$large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $lead->ID ), 'thumbnail' );	
   						...
   						if ( ! empty( $large_image_url[0] ) ) {
        					...
        					//	echo '</a>';
                            	?>
				            <a href='/?p=<?php echo $lead->ID; ?>'> <img src='<?php echo esc_url( $large_image_url[0] ); ?>' class='1img-responsive 1aligncenter img-thumbnail ads ads-shift'></a>
				<?php
   					 	}
					}
				echo "</div>";
			...
			<div class='col-sm-7'>
				...
				<?php
					...
					if( get_user_meta($lead->post_author, 'MX_user_onlyfans' , 1) ){
						$fans_name = get_user_meta($lead->post_author, 'MX_user_onlyfans' , 1);
						
						
						
						echo ' <span  data-toggle="tooltip" title="Private Message" class="btn btn-default  hidden1 glyphicon glyphicon-envelope"></span> ';
						?>
						<a target="_blank" href="https://onlyfans.com/<?php echo $fans_name; ?>">
						    <img src="https://dlfreakfest.org/wp-content/themes/ssi2017/images/icons/icon-onlyfans.png" width="25" height="25"> <?php echo $fans_name; ?>
						</a>
